...
class CommentsFactory {
  static createCommentsComponents(prisma) {
...
const { commentsController } = CommentsFactory.createCommentsComponents(prisma);


/** 1. 댓글 작성 API */
router.post('/:postId/comments', authMiddleware, commentsController.createComment);

/** 2. 댓글 (전체)조회 API */ // - 해당 post(by postId)에 작성된 '전체'댓글 조회
